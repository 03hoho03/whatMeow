...
# Comment 삭제 함수 구현
async def delete_comment(comment_id, user_id, db):
    try:
        c_row = db.query(model.Comment).filter_by(id=comment_id, uploader=user_id).first()
        db.delete(c_row)
        db.commit()
        return True
...
# Comment 업데이트 함수 구현
async def update_comment(new_comment, comment_id, user_id, db):
    try:
        c_row = db.query(model.Comment).filter_by(id=comment_id, uploader=user_id).first()
