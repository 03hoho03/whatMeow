...
for class_name in classes:
  image_lst = os.listdir(f'images/{class_name}') # class_name이라는 이름을 가진 폴더 내의 사진이름들의 List 반환
...
for epoch in range(num_epochs):
  ...
  for inputs,labels in  tqdm(train_dataloader,desc="Train : ",total=len(train_dataloader)):
  ...
  model.eval()
  with torch.no_grad():
    for data in tqdm(test_dataloader,desc="Val : ", total=len(test_dataloader)):
      inputs, labels = data
      inputs = inputs.to(device)
      labels = labels.to(device)
...
for i,accuracy in enumerate(accuracies):
  print(f"{i+1}번 째 Epoch 정확도 : {accuracy}%")
